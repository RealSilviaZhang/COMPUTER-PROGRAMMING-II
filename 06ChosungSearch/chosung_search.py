...
def chosung_indexing(cho_indexing, filename):
    ...
    for i in ls:
        cho_indexing = defaultdict(list)
        k = str(str2chostr(i[0]))
        cho_indexing[k].append(i)
        cho_indexing[k].sort(key=lambda x: map(int, itemgetter(1, 0)(x)), reverse=True)
